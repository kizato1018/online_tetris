...
class Table():
    def __init__(self, height, width):
        self.width = width
        self.height = height
        self.bufferedTable = [[Block("grey") for i in range(width + 10)]for j in range(height + 10)]
        
    def get_real_table(self):
        table = [[Block("grey") for i in range(self.width)]for j in range(self.height)]
...
class Brick:
    def __init__(self, type):
        ...
        color = "#" + "".join(map(hex, list(np.random.choice(range(256), size=3))))
        if(type == "I"):
